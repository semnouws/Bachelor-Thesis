...
# ----ROBUSTNESS TESTS-------------------------------------------------------------------------------
def test_ballot_noise(ballots, parties, n_trials=100, noise_std=0.5):
    """Add small noise to prob scores and check winner stability."""
    results = {sys: [] for sys in ['Condorcet', 'Borda', 'Ranked', 'IRV']}

    for _ in range(n_trials):
        noisy_ballots = []
        for ballot in ballots:
            noisy = {p: max(0, s + np.random.normal(0, noise_std))
                     for p, s in ballot.items()}
            noisy_ballots.append(noisy)

        results['Condorcet'].append(condorcet_ballots(noisy_ballots, parties)['winner'])
        results['Borda'].append(borda_ballots(noisy_ballots, parties)['winner'])
        results['Ranked'].append(ranked_voting_ballots(noisy_ballots, parties)['winner'])
        results['IRV'].append(instant_runoff_ballots(noisy_ballots, parties)['winner'])

    print(f"\n=== Noise Robustness (noise_std={noise_std}, {n_trials} trials) ===")
    for sys, winners in results.items():
        counts = pd.Series(winners).value_counts()
        dominant = counts.index[0]
        stability = counts.iloc[0] / n_trials * 100
        print(f"  {sys:<12} → {dominant:<20} wins {stability:.0f}% of trials")
        if stability < 80:
            print(f"             UNSTABLE — also won by: {dict(counts.iloc[1:].items())}")

def test_ballot_subsampling(ballots, parties, date=None, fractions=[0.5, 0.7, 0.9], n_trials=50):
    """Check if winner is stable when only a subset of ballots are used."""
    print(f"\n=== Subsampling Robustness ({n_trials} trials per fraction) ===")

    for frac in fractions:
        n = max(10, int(len(ballots) * frac))
        winners = {'Condorcet': [], 'Borda': [], 'Ranked': [], 'IRV': []}

        for _ in range(n_trials):
            sample = np.random.choice(len(ballots), n, replace=False)
            sub = [ballots[i] for i in sample]

            winners['Condorcet'].append(condorcet_ballots(sub, parties)['winner'])
            winners['Borda'].append(borda_ballots(sub, parties)['winner'])
            winners['Ranked'].append(ranked_voting_ballots(sub, parties)['winner'])
            winners['IRV'].append(instant_runoff_ballots(sub, parties, date=date, actual_by_date=actual_by_date)['winner'])

        print(f"\n  Fraction {frac:.0%} ({n} ballots):")
        for sys, ws in winners.items():
            counts = pd.Series(ws).value_counts()
            stability = counts.iloc[0] / n_trials * 100
            print(f"    {sys:<12} → {counts.index[0]:<20} {stability:.0f}% stable")
...
def run_per_election(df_results: pd.DataFrame):
    ...
    for i, (date, group) in enumerate(df_results.groupby('Date')):
        ...
        if use_ballots and i == 0:
            print(f"\n--- Robustness Tests (NKO {nko_year}, applied to all elections) ---")
            test_system_agreement(ballots, all_parties, date=date)
            test_ballot_noise(ballots, all_parties)
            test_ballot_subsampling(ballots, all_parties, date=date)
